...
# Added on 9 Aug - Ankita: the test split is persisted too, so training can
...
# A wider vocabulary was tried 10 Aug - Ankita to help the weak NEUTRAL class: 30k
...
def build_tfidf_features(test_size=TEST_SIZE, random_state=RANDOM_STATE):
    ...
    y = df["sentiment"]
    # Added by Ankita 11 Aug: carry the raw text through too, so a trainer that
    ...
    train_df["full_review"] = full_train  # Added by Ankita 11 Aug
    train_df["sentiment"] = y_train
    train_df.to_csv(TRAIN_CSV, index=False)

    test_df = X_test.to_frame(name="clean_review")
    test_df["full_review"] = full_test  # Added by Ankita 11 Aug
